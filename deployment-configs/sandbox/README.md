...
The "sandbox" folder is organized to categorize various components and configurations for sandbox environments.

# Baseline-Infra

Baseline infrastructure configurations and components.

## Usage of `bicep-pipeline-sandbox.yml`

This README provides an overview of the [`bicep-pipeline-sandbox.yml`](https://github.com/devsecops-len/template-deployment-config-repo/blob/release/deployment-configs/sandbox/baseline-infra/azure-pipelines/bicep-pipeline-sandbox.yml) file, an Azure Pipeline YAML configuration for deploying sandbox infrastructure using Bicep templates.
...
4. Utilize the [`pipeline-api-components.yaml`](https://github.com/devsecops-len/template-deployment-config-repo/blob/release/deployment-configs/sandbox/apim/bff/v1/pipeline-api-components.yaml) file to configure deployment pipelines for the API components of that version.
...
The [`loadtest-trigger.yaml`](https://github.com/devsecops-len/template-deployment-config-repo/blob/release/deployment-configs/sandbox/load-test/azure-pipelines/loadtest-trigger.yaml) pipeline is designed to deploy and trigger all load tests for the project. It is scoped to provide control over deployment and triggers for load tests across different environments, such as development (dev), quality assurance (qa), user acceptance testing (uat), and production (prod).
